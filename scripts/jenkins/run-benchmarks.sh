...
function benchmark() {
    ...
    sudo -n cset proc --exec /benchmark -- \
        ...
        -prof gc \
        -prof com.atatus.apm.agent.benchmark.profiler.ReporterProfiler \
        -rf json \
        -rff ${RESULT_FILE}
    ...
    $JAVA_HOME/bin/java -cp apm-agent-benchmarks/target/benchmarks.jar com.atatus.apm.agent.benchmark.PostProcessBenchmarkResults ${RESULT_FILE} ${BULK_UPLOAD_FILE} ${COMMIT_UNIX}
}

...
.build.gradle
----
compile "com.atatus.apm:apm-agent-api:$elasticApmVersion"
----

Replace the version placeholders with the
link:https://search.maven.org/search?q=g:com.atatus.apm%20AND%20a:apm-agent-api[
latest version from maven central]:
image:https://img.shields.io/maven-central/v/com.atatus.apm/apm-agent-api.svg[Maven Central]
...
To use the API, you can just invoke the static methods on the class `com.atatus.apm.api.AtatusApm`.
...
Transaction transaction = AtatusApm.currentTransaction();
----

NOTE: Transactions created via <<api-start-transaction, `AtatusApm.startTransaction()`>>
...
NOTE: Transactions created via this method can not be retrieved by calling <<api-current-span, `AtatusApm.currentSpan()`>>
or <<api-current-transaction, `AtatusApm.currentTransaction()`>>.
...
See <<api-start-transaction, `AtatusApm.startTransaction()`>>, <<api-transaction-activate, `transaction.activate()`>>
...
add `AtatusApm.currentTransaction()` with the key `"transaction"` to the model.
...
NOTE: Spans created via this method can not be retrieved by calling <<api-current-span, `AtatusApm.currentSpan()`>>.
See <<api-span-activate, `span.activate()`>> on how to achieve that.
...
// on the current thread via AtatusApm.currentTransaction().
...
// on the current thread via AtatusApm.currentSpan().
// This is also true for methods called within the try block.

...
.build.gradle
----
compile "com.atatus.apm:atatus-agent-api:$elasticApmVersion"
----

Replace the version placeholders with the
link:https://search.maven.org/search?q=g:com.atatus.apm%20AND%20a:atatus-agent-api[
latest version from maven central]:
image:https://img.shields.io/maven-central/v/com.atatus.apm/atatus-agent-api.svg[Maven Central]
...
To use the API, you can just invoke the static methods on the class `com.atatus.apm.api.Atatus`.
...
NOTE: Transactions created via <<api-start-transaction, `Atatus.startTransaction()`>>
...
NOTE: Transactions created via this method can not be retrieved by calling <<api-current-span, `Atatus.currentSpan()`>>
or <<api-current-transaction, `Atatus.currentTransaction()`>>.
...
See <<api-start-transaction, `Atatus.startTransaction()`>>, <<api-transaction-activate, `transaction.activate()`>>
...
add `Atatus.currentTransaction()` with the key `"transaction"` to the model.
...
NOTE: Spans created via this method can not be retrieved by calling <<api-current-span, `Atatus.currentSpan()`>>.
See <<api-span-activate, `span.activate()`>> on how to achieve that.
...
// on the current thread via Atatus.currentTransaction().
...
// on the current thread via Atatus.currentSpan().
// This is also true for methods called within the try block.
